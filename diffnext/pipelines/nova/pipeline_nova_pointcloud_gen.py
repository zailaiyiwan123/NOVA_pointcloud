...
##############################################################################
"""NOVA Point Cloud Generation Pipeline based on NOVA architecture."""
...
class NOVAPointCloudGenerationPipeline(DiffusionPipeline, PipelineMixin):
    """Improved NOVA pipeline for 3D point cloud generation from text prompts."""
    ...
    def __call__(
        ...
        **kwargs,
        ...
        """Generate 3D point clouds from text prompts using NOVA architecture.
        ...
        # Generate point clouds using NOVA's diffusion loop
        point_clouds = []
        colors = []
        ...
        for i in range(batch_size * num_point_clouds_per_prompt):
            point_cloud = self.latents_to_point_cloud(
                latents[i:i+1],
    ...
    def latents_to_point_cloud(self, latents: torch.Tensor, num_points: int = 15000) -> torch.Tensor:
        """Convert latents to point cloud coordinates (similar to NOVA's decoding)."""
